...
O(𝒯⁴) DC current restricted to a SPECIFIC pair of energy-exchange channels labelled
by harmonic indices `ao`, `bo`: across the four tunnelling vertices the net
exchanged quanta are fixed to {+ao, +bo, −ao, −bo}·Ω, and the code sums over all
vertex permutations (`Sar`) of these. Lets one dissect an individual (ao,bo)
multiple-Andreev-reflection channel's contribution to the order-𝒯⁴ current. Uses
the full BCS surface GF from [`surfacegr`](@ref).

`War`: Fourier coefficients of `e^{iφ/2}`, ordered `+nf:-2:-nf` with `nf=max(ao,bo)`.
Returns `2·I_LR` (the factor 2 accounts for the equal-and-opposite LR/RL parts).
"""
...
"""
    current_Vbias_MPT_T4_aobo1(war1, Omega, zeta, delta, T, Gamma, ao, bo, War) -> Float64

Alternative formulation of [`current_Vbias_MPT_T4_aobo`](@ref) written in terms of
the lead spectral functions `A(ω)=−(gʳ−gᵃ)/2πi` (diagonal) and `B(ω)=(gʳ+gᵃ)/2`
(anomalous), with the Fermi-factor step functions written out explicitly. Gives the
same (ao,bo)-channel O(𝒯⁴) pair-current contribution in a form that exposes the
quasiparticle occupation/threshold structure.
"""
function current_Vbias_MPT_T4_aobo1(war1, Omega, zeta, delta, T, Gamma, ao, bo, War) #War: Fourier coefficients of exp(i*phi/2), in the order +nf:-2:-nf, where nf = maximum([ao, bo])
    Nw1 = length(war1); deltaw1 = abs(war1[2]-war1[1]); tau3 = [1 0; 0 -1];
    ...
    Idcw = zeros(Float64,Nw1);
    Threads.@threads for hi = 1:Nw1
        Aar = zeros(ComplexF64, 2*(ao+bo)+1);
        Bar = zeros(ComplexF64, 2*(ao+bo)+1);

        for ab = -(ao+bo):(ao+bo)
            wwab = war1[hi] + ab*Omega;
            grw = Keldyshsetup_Floquetn.surfacegr(zeta, delta, Gamma, wwab, 1);
           
            Aa = grw - grw';
            Aar[(ao+bo)+1-ab] = -(Aa[1,1])/(2*im*pi);
            Ba = grw + grw';
            Bar[(ao+bo)+1-ab] = (Ba[1,2])/(2);
        end

        Idcw[hi] = Idcw[hi] + real( Aar[(ao+bo)+1+ao]*Aar[(ao+bo)+1-bo]*Bar[(ao+bo)+1]*Bar[(ao+bo)+1] * ( (war1[hi]-ao*Omega .< 0) - (war1[hi]+bo*Omega .< 0) ) );
        Idcw[hi] = Idcw[hi] + real( Aar[(ao+bo)+1+bo]*Aar[(ao+bo)+1-ao]*Bar[(ao+bo)+1]*Bar[(ao+bo)+1] * ( (war1[hi]-bo*Omega .< 0) - (war1[hi]+ao*Omega .< 0) ) );
        # Idcw[hi] = Idcw[hi] - real( Aar[(ao+bo)+1-ao]*Aar[(ao+bo)+1+bo]*Bar[(ao+bo)+1]*Bar[(ao+bo)+1] * ( (war1[hi]+ao*Omega .< 0) - (war1[hi]-bo*Omega .< 0) ) );
        # Idcw[hi] = Idcw[hi] - real( Aar[(ao+bo)+1-bo]*Aar[(ao+bo)+1+ao]*Bar[(ao+bo)+1]*Bar[(ao+bo)+1] * ( (war1[hi]+bo*Omega .< 0) - (war1[hi]-ao*Omega .< 0) ) );
        Idcw[hi] = Idcw[hi] + real( Aar[(ao+bo)+1+ao]*Aar[(ao+bo)+1+bo]*Bar[(ao+bo)+1]*Bar[(ao+bo)+1] * ( (war1[hi]-ao*Omega .< 0) - (war1[hi]-bo*Omega .< 0) ) );
        Idcw[hi] = Idcw[hi] + real( Aar[(ao+bo)+1+bo]*Aar[(ao+bo)+1+ao]*Bar[(ao+bo)+1]*Bar[(ao+bo)+1] * ( (war1[hi]-bo*Omega .< 0) - (war1[hi]-ao*Omega .< 0) ) );
    ...
    # Idcw = zeros(Float64,Nw1);
    # Threads.@threads for hi = 1:Nw1
    #     wwu = war1[hi] - bo*Omega;
    #     grwu = Keldyshsetup_Floquetn.surfacegr(zeta, delta, Gamma, wwu, 1);
    #     Aau = grwu - grwu';
    #     Aaru = -(Aau[1,1])/(2*im*pi);
        
    #     wwd = war1[hi] - ao*Omega;
    #     grwd = Keldyshsetup_Floquetn.surfacegr(zeta, delta, Gamma, wwd, 1);
    #     Aad = grwd - grwd';
    #     Aard = -(Aad[1,1])/(2*im*pi);
        
    #     wwm = war1[hi];
    #     grwm = Keldyshsetup_Floquetn.surfacegr(zeta, delta, Gamma, wwm, 1);
    #     Bam = grwm + grwm';
    #     Barm = (Bam[1,2])/(2);
        
    #     Idcw[hi] = Idcw[hi] + real( Aard*Aaru*Barm*Barm * ( (wwd .< 0) - (wwu .< 0) ) );
    # end
    # plot(war1, Idcw)
    # xlims!(-3,3)
...
"""
    current_Vbias_MPT_T4_fghfhg_mbomaoboao(war1, Omega, zeta, delta, T, Gamma, ao, bo, War) -> Float64

O(𝒯⁴), (ao,bo)-channel current for ONE specific fixed vertex/energy ordering rather
than the full permutation sum of [`current_Vbias_MPT_T4_aobo`](@ref). The suffix
`mbomaoboao` encodes the sequence of energy shifts (−bo, −ao, +bo, +ao) along the
propagator chain; isolates an individual time-ordered diagram. Companion of
[`current_Vbias_MPT_T4_fghfhg_maomboboao`](@ref).
"""
function current_Vbias_MPT_T4_fghfhg_mbomaoboao(war1, Omega, zeta, delta, T, Gamma, ao, bo, War) #War: Fourier coefficients of exp(i*phi/2), in the order +nf:-2:-nf, where nf = maximum([ao, bo])
    Nw1 = length(war1); deltaw1 = abs(war1[2]-war1[1]); tau3 = [1 0; 0 -1];
    ...
    Threads.@threads for hi = 1:Nw1
        ...
        Idcw[hi] = Idcw[hi] + T^4 .* real( conj(War[nf+1-(-3)]) * grwar[(ao+bo)+1-3,1,2] * conj(War[nf+1-(-1)]) * grwar[(ao+bo)+1-1-3,2,2] * War[nf+1-3] * grwar[(ao+bo)+1-1,2,1] * War[nf+1-1] * glwar[(ao+bo)+1-0,1,1] ) +
                              T^4 .* real( conj(War[nf+1-(-3)]) * grwar[(ao+bo)+1-3,1,2] * conj(War[nf+1-(-1)]) * grwar[(ao+bo)+1-1-3,2,2] * War[nf+1-3] * glwar[(ao+bo)+1-1,2,1] * War[nf+1-1] * gawar[(ao+bo)+1-0,1,1] ) + 
                              T^4 .* real( conj(War[nf+1-(-3)]) * grwar[(ao+bo)+1-3,1,2] * conj(War[nf+1-(-1)]) * glwar[(ao+bo)+1-1-3,2,2] * War[nf+1-3] * gawar[(ao+bo)+1-1,2,1] * War[nf+1-1] * gawar[(ao+bo)+1-0,1,1] ) +
                              T^4 .* real( conj(War[nf+1-(-3)]) * glwar[(ao+bo)+1-3,1,2] * conj(War[nf+1-(-1)]) * gawar[(ao+bo)+1-1-3,2,2] * War[nf+1-3] * gawar[(ao+bo)+1-1,2,1] * War[nf+1-1] * gawar[(ao+bo)+1-0,1,1] );
...
"""
    current_Vbias_MPT_T4_fghfhg_maomboboao(war1, Omega, zeta, delta, T, Gamma, ao, bo, War) -> Float64

O(𝒯⁴), (ao,bo)-channel current for the fixed vertex/energy ordering (−ao, −bo, +bo,
+ao) (suffix `maomboboao`); the single-diagram companion of
[`current_Vbias_MPT_T4_fghfhg_mbomaoboao`](@ref).
"""
function current_Vbias_MPT_T4_fghfhg_maomboboao(war1, Omega, zeta, delta, T, Gamma, ao, bo, War) #War: Fourier coefficients of exp(i*phi/2), in the order +nf:-2:-nf, where nf = maximum([ao, bo])
    Nw1 = length(war1); deltaw1 = abs(war1[2]-war1[1]); tau3 = [1 0; 0 -1];
    ...
    Threads.@threads for hi = 1:Nw1
        ...
        Idcw[hi] = Idcw[hi] + T^4 .* real( conj(War[nf+1-(-1)]) * grwar[(ao+bo)+1-1,1,2] * conj(War[nf+1-(-3)]) * grwar[(ao+bo)+1-1-3,2,2] * War[nf+1-3] * grwar[(ao+bo)+1-1,2,1] * War[nf+1-1] * glwar[(ao+bo)+1-0,1,1] ) +
                              T^4 .* real( conj(War[nf+1-(-1)]) * grwar[(ao+bo)+1-1,1,2] * conj(War[nf+1-(-3)]) * grwar[(ao+bo)+1-1-3,2,2] * War[nf+1-3] * glwar[(ao+bo)+1-1,2,1] * War[nf+1-1] * gawar[(ao+bo)+1-0,1,1] ) +   
                              T^4 .* real( conj(War[nf+1-(-1)]) * grwar[(ao+bo)+1-1,1,2] * conj(War[nf+1-(-3)]) * glwar[(ao+bo)+1-1-3,2,2] * War[nf+1-3] * gawar[(ao+bo)+1-1,2,1] * War[nf+1-1] * gawar[(ao+bo)+1-0,1,1] ) +
                              T^4 .* real( conj(War[nf+1-(-1)]) * glwar[(ao+bo)+1-1,1,2] * conj(War[nf+1-(-3)]) * gawar[(ao+bo)+1-1-3,2,2] * War[nf+1-3] * gawar[(ao+bo)+1-1,2,1] * War[nf+1-1] * gawar[(ao+bo)+1-0,1,1] );
...
"""
    Siglf(ww, Omega, Nf, zeta, delta, T, Gamma, Sigl_b, Sigl_s) -> Matrix{ComplexF64}

Lesser self-energy Σ< in the Floquet–Nambu–lead basis, with two additive
contributions selected by flags:
- `Sigl_b == 1`: local equilibrium baths attached to each site — `2iΓ` on occupied
  Floquet modes.
- `Sigl_s == 1`: embeds the semi-infinite-lead occupation into the contact site via
  the surface term `ζ²·τ₃ g< τ₃` (method 1); `Sigl_s == 2` uses an alternative
  retarded-self-energy-difference embedding (method 2).
Supplies the occupation information for the Keldysh equation in
[`Glesser_Floquet_Tfull`](@ref).
"""
function Siglf(ww, Omega, Nf, zeta, delta, T, Gamma, Sigl_b, Sigl_s)
...
"""
    Glesser_Floquet_Tfull(ww, Omega, Nf, zeta, delta, T, Gamma, Vip, Sigl_b, Sigl_s,
...
"""
function Glesser_Floquet_Tfull(ww, Omega, Nf, zeta, delta, T, Gamma, Vip, Sigl_b, Sigl_s, Grwmn = nothing, Gawmn = nothing, Sigl = nothing)
    if isnothing(Grwmn)
    ...
    if isnothing(Sigl)
        Sigl = Keldyshsetup_Floquetn.Siglf(ww, Omega, Nf, zeta, delta, T, Gamma, Sigl_b, Sigl_s);
    end
...
"""
    current_Floquet_Tfull(war0, Omega, Nf, zeta, delta, T, Gamma, Vip, Sigl_b, Sigl_s, iterprint) -> Matrix{ComplexF64}
...
"""
function current_Floquet_Tfull(war0, Omega, Nf, zeta, delta, T, Gamma, Vip, Sigl_b, Sigl_s, iterprint)
    ...
    Threads.@threads for ij = 1:Nw0
        # println(ij)
        Glwmn = Keldyshsetup_Floquetn.Glesser_Floquet_Tfull(war0[ij], Omega, Nf, zeta, delta, T, Gamma, Vip, Sigl_b, Sigl_s)
...
"""
    IbiasResidual_Tfull(Vipi, war0, Omega, Nf, zeta, delta, T, Gamma, Sigl_b, Sigl_s, iterprint) -> Vector{Float64}
...
"""
function IbiasResidual_Tfull(Vipi, war0, Omega, Nf, zeta, delta, T, Gamma, Sigl_b, Sigl_s, iterprint)
    ...
    #--Current--
    Iif = Keldyshsetup_Floquetn.current_Floquet_Tfull(war0, Omega, Nf, zeta, delta, T, Gamma, Vip, Sigl_b, Sigl_s, iterprint);
    Ifa = zeros(ComplexF64, 4*Nf+1);
    for kl = -Nf:Nf
        for lm = -Nf:Nf
...
function IbiasResidual_T6(Vipi, war0, Omega, Nf, zeta, delta, T, Gamma, iterprint)
    ...
    #--Current--
    Iif = Keldyshsetup_Floquetn.current_Floquet_T6(war0, Omega, Nf, zeta, delta, T, Gamma, Vip, iterprint);
    Ifa = zeros(ComplexF64, 4*Nf+1);
    for kl = -Nf:Nf
        for lm = -Nf:Nf
            Ifa[-(kl-lm)+(2*Nf+1)] = Ifa[-(kl-lm)+(2*Nf+1)] + Iif[-kl+Nf+1,-lm+Nf+1];
...
function IbiasResidual_T8(Vipi, war0, Omega, Nf, zeta, delta, T, Gamma, iterprint)
    ...
    #--Current--
    Iif = Keldyshsetup_Floquetn.current_Floquet_T8(war0, Omega, Nf, zeta, delta, T, Gamma, Vip, iterprint);
    Ifa = zeros(ComplexF64, 4*Nf+1);
    for kl = -Nf:Nf
        for lm = -Nf:Nf
            Ifa[-(kl-lm)+(2*Nf+1)] = Ifa[-(kl-lm)+(2*Nf+1)] + Iif[-kl+Nf+1,-lm+Nf+1];
...
"""
    IbiasJacobian_Tfull(Vipi, war0, Omega, Nf, zeta, delta, T, Gamma, Sigl_b, Sigl_s, iterprint) -> Matrix{Float64}
...
"""
function IbiasJacobian_Tfull(Vipi, war0, Omega, Nf, zeta, delta, T, Gamma, Sigl_b, Sigl_s, iterprint)
    ...
    for gh = 1:Nw0
        ...
        # Propagators at this frequency, shared by all 4Nf column-derivatives (built once per ω):
        ...
        Sigl = Siglf(war0[gh], Omega, Nf, zeta, delta, T, Gamma, Sigl_b, Sigl_s);  # lesser self-energy Σ<
        ...
        # ∂(current operator)/∂x_ij. The current is tr[-Vᵢ·G<]; since Σ< is x-independent
        # and V is linear in x, ∂G< = Gr·M_ij·G< + G<·M_ij·Ga, giving the three terms below.
        # Columns ij are independent ⇒ threaded; t1,t2,t3 are thread-local scratch from tls.
        Threads.@threads for ij = 1:(4*Nf) #variable (for derivative in Jacobian) index
            ...
            mul!(t3, Miarsp[ij], Glwmn, 1, 1) # + (-∂Vᵢ/∂x) · G<  (sparse, O(N²))
...
function IbiasJacobian_T6(Vipi, war0, Omega, Nf, zeta, delta, T, Gamma, iterprint)
    ...
    for gh = 1:Nw0
        ...
        Glwmn_w6 .= Keldyshsetup_Floquetn.Glesser_Floquet_T6(war0[gh], Omega, Nf, zeta, delta, T, Gamma, Vip, grwmn, gawmn, glwmn); #Not calculated repeatedly for each variable        
...
function IbiasJacobian_T8(Vipi, war0, Omega, Nf, zeta, delta, T, Gamma, iterprint)
    ...
    for gh = 1:Nw0
        ...
        gawmn = conj(transpose( grwmn ));
        glwmn = Keldyshsetup_Floquetn.glwmnf(war0[gh], Omega, Nf, zeta, delta, Gamma)
        Glwmn_w8 .= Keldyshsetup_Floquetn.Glesser_Floquet_T8(war0[gh], Omega, Nf, zeta, delta, T, Gamma, Vip, grwmn, gawmn, glwmn); #Not calculated repeatedly for each variable        
...
"""
    phisolve(ws, dw0, evar, Nf, zeta, delta, T, Gamma, Sigl_b, Sigl_s,
...
"""
function phisolve(ws, dw0, evar, Nf, zeta, delta, T, Gamma, Sigl_b, Sigl_s, Vipsolseed = nothing, Nevseed = nothing)
    Nev = length(evar);
    ...
    for hi = Nev:-1:1
        ...
        if ws == 8
            fvalue = norm(Keldyshsetup_Floquetn.IbiasResidual_T8(Vipseed, war0, Omega, Nf, zeta, delta, T, Gamma,  hi));
        elseif ws == 6
            fvalue = norm(Keldyshsetup_Floquetn.IbiasResidual_T6(Vipseed, war0, Omega, Nf, zeta, delta, T, Gamma,  hi));
        elseif ws == 4
        ...
        elseif ws == 0
            fvalue = norm(Keldyshsetup_Floquetn.IbiasResidual_Tfull(Vipseed, war0, Omega, Nf, zeta, delta, T, Gamma, Sigl_b, Sigl_s, hi));
        end
        ...
        if fvalue < ftols
            ...
            if ws == 8
                res = nlsolve(x -> Keldyshsetup_Floquetn.IbiasResidual_T8(x, war0, Omega, Nf, zeta, delta, T, Gamma, hi), Vipseed, show_trace=true, ftol = ftols; xtol = xtols, iterations = itermax);
            elseif ws == 6
                res = nlsolve(x -> Keldyshsetup_Floquetn.IbiasResidual_T6(x, war0, Omega, Nf, zeta, delta, T, Gamma, hi), Vipseed, show_trace=true, ftol = ftols; xtol = xtols, iterations = itermax);
            elseif ws == 4
            ...
            elseif ws == 0 
                res = nlsolve(x -> Keldyshsetup_Floquetn.IbiasResidual_Tfull(x, war0, Omega, Nf, zeta, delta, T, Gamma, Sigl_b, Sigl_s, hi), x -> Keldyshsetup_Floquetn.IbiasJacobian_Tfull(x, war0, Omega, Nf, zeta, delta, T, Gamma, Sigl_b, Sigl_s, hi), Vipseed, show_trace=true, method = :trust_region, ftol = ftols; xtol = xtols, iterations = itermax);
            end
        ...
        elseif ws == 0 
            If[hi,:,:] = Keldyshsetup_Floquetn.current_Floquet_Tfull(war0, Omega, Nf, zeta, delta, T, Gamma, VipI, Sigl_b, Sigl_s, hi);
        end
